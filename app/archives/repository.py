    ...
    def create(self, data: dict[str, Any], now: str) -> dict[str, Any]:
        cursor = self.connection.execute(
            """INSERT INTO dossiers(dossier_code,intake_id,disclosure_event_id,source_dossier_id,root_dossier_id,asset_type,
               quantity,unit,lifecycle_state,vault_id,custody_user_id,secrecy_level,publication_state,provenance_depth,created_at,updated_at)
               VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (
                ...
                data.get("vault_id"), data.get("custody_user_id"),
                data.get("secrecy_level", "internal"), data.get("publication_state", "unpublished"),
